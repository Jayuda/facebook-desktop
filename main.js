...
chrome.app.runtime.onLaunched.addListener(function(launchData) {
  ...
  if (launchData.id) {
    ...
    if (launchData.id === 'facebook_dektop') {
      // Convert the desktop URL to a mobile one.
      articleUrl = launchData.url.replace("www.facebook.com", "m.facebook.com");
    } else if (launchData.id === 'facebook_mobile') {
      articleUrl = launchData.url;
    } else {
      console.error("Unexpected URL handler ID: " + launchData.id);
    }
  } else {
    articleUrl = "https://www.facebook.com/";
  }
